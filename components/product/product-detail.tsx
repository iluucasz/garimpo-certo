    ...
    <section className="container product-hero-section">
      ...
      <div className="product-detail">
        ...
        <div className="pdp-copy">
          ...
          <h1>{product.name}</h1>
          <p className="pdp-rating"><span><Star fill="currentColor" /> {product.rating.toLocaleString('pt-BR', { minimumFractionDigits: 1 })}</span>{bestOffer?.soldCount ? <span>{formatSoldCount(bestOffer.soldCount)}{bestProvider ? ` na ${bestProvider.name}` : ''}</span> : null}</p>
          ...
          {bestOffer && bestProvider && <div className="primary-offer-card">
            ...
            <div className="primary-offer-price"><div><s>{previousPrice > bestOffer.price ? formatPrice(previousPrice) : '\u00a0'}</s><strong>{formatPrice(bestOffer.price)}</strong>{saving > 0 && <span>Economize {formatPrice(saving)}</span>}</div><div className="primary-offer-details"><span><Truck /> Frete calculado na Shopee</span>{bestOffer.soldCount ? <span>{formatSoldCount(bestOffer.soldCount)} na {bestProvider.name}</span> : null}<span>{bestOffer.stock} · atualizado {bestOffer.updatedAt}</span></div></div>
            ...
            <small className="total-note">Preço de <strong>{formatPrice(total)}</strong> na última sincronização com a Shopee. Preço, frete e estoque podem mudar lá.</small>
          </div>}
