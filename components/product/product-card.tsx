...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    ...
    <div className="product-info">
      <div className="product-card-meta"><span>{product.brand}</span><span className="rating"><Star fill="currentColor" aria-hidden="true"/>{product.rating.toLocaleString('pt-BR', { minimumFractionDigits: 1 })}<span className="sr-only">de 5 estrelas</span></span></div>
      <h3>{product.name}</h3>
      <div className="product-price-block">{offer ? <div><strong>{formatPrice(offer.price)}</strong>{previous > offer.price && <s>{formatPrice(previous)}</s>}</div> : <span className="unavailable-offer">Sem oferta disponível</span>}</div>
      {provider && <div className="product-offer-source"><span>na {provider.name}</span>{offer?.soldCount ? <small className="product-sold-count">{formatSoldCount(offer.soldCount)}</small> : null}</div>}
