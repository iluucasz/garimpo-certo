...
- `components/admin/experience-studio.tsx` (ranking/recommendation-slot/segment simulators) — admin-only demo tooling, not customer-facing.
...
- `pnpm db:generate`/`db:migrate`/`db:pull`/`db:studio`/`db:seed-roles`/`db:grant-role`/`db:seed-catalog` are all wired. Migration tracking is baselined against the pre-existing schema — `db:migrate` is safe/idempotent.
