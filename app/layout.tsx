...
export default async function RootLayout({children}:{children:React.ReactNode}){const catalog=await getStoreCatalog();return <html lang="pt-BR" className="bg-background"><body className={`${bodyFont.variable} ${displayFont.variable} font-sans antialiased`}><StoreDataProvider data={catalog}><MockProvider>{children}</MockProvider></StoreDataProvider>{process.env.NODE_ENV==='production'&&<Analytics/>}</body></html>}
